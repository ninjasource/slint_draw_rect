...
use log::*;
use slint_backend::{DISPLAY_HEIGHT, DISPLAY_WIDTH, StmBackend, TargetPixelType};
...
use crate::dma2d_logger::Dma2DBuffer;
    ...
    loop {
        ...
        let _is_dirty = window.draw_if_needed(|renderer| match pool.try_pull() {
            Some(mut buffer) => {
                ...
                renderer.render_into_buffer(&mut Dma2DBuffer::new(&mut buffer, DISPLAY_WIDTH));
